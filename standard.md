...
> -   使用`z-index`属性尽量`z-index`的值不要超过1500，页面中的元素内容的`z-index`不能超过500（提示框等模块除外但维持在1500以下）。
> -   尽量避免使用CSS Hack。
>
...
### 注释规范：
-------------------
> -   `javascript`、`css`文件注释需要标明文件版本、创建/修改时间、重大版本修改记录、函数描述、文件版本、创建或者修改时间、功能、作者等信息。
>
		/* * 注释块 */
>     中间可添加如下信息。
>
...
-------------------
> -   编码格式化，三码统一。
> -   测试工具: 前期开发仅测试FireFox & Chrome & Safari & Opera  & IE8 & IE9 & IE10
