...
//				success: function(response) {
					...
					response.data=[
						...
						{listId:'1002',productName:'F19S-智能飞行电池',imageUrl:'../images/product/f19s-dc1!60x60.jpg',productUrl:"product-buy-acce.html",salePrice:'299',subtotal:'598',selStatus:1,num:5,buyLimit:5}
					];
					//*******end********
			...
			if(defaultCart.cartData.length<=0){
				...
				$.each(defaultCart.cartData, function(i,t) { 
					...
					if(1===t.selStatus){
						...
						defaultCart.totalPrice+=parseInt(t.subtotal)*parseInt(t.num)
					}
				});
			...
			if(id.length){
				JNC.confirm({
				    ...
				    buttons:{
				        ok:{
				        	action: function(){
				        		console.log("Delete the success")
				        	}
				        }
				    }
				});
